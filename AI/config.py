...
- BIẾN MÔI TRƯỜNG:
  ...
  - OPENAI_API_KEY: API key OpenAI (chat)
...
# Model mặc định OpenAI (override bằng OPENAI_MODEL)
...
EXTRACT_SYSTEM_PROMPT = """Bạn trích xuất thông tin từ câu hỏi tiếng Việt hoặc tiếng Anh về công việc/task/dự án.

Trả lời ĐÚNG MỘT dòng JSON (UTF-8), không markdown, không ```, không giải thích thêm.

Các khóa (luôn có đủ 3 khóa):
- "project_name": chuỗi — tên hoặc từ khóa gọi dự án khi user nhắc tới (kể cả tên ngắn như "taskmate", "mobile"). 
  QUAN TRỌNG: Nếu câu có dạng "project X", "dự án X", "của project X", "việc của dự án X" thì PHẢI điền X vào project_name (bỏ chữ "project"/"dự án" ở giá trị).
- "task_status": một trong ba giá trị chính xác: "to do", "pending", "done", hoặc "" nếu user không hỏi theo trạng thái.
  Ánh xạ tiếng Việt: chưa làm/todo/cần làm → "to do"; đang làm/in progress/pending → "pending"; xong/hoàn thành/done → "done".
- "task_assignee": tên người (họ tên hoặc tên gọi) nếu có, không thì "".

Ví dụ:
User: "những việc cần làm của project taskmate là gì"
→ {"project_name":"taskmate","task_status":"","task_assignee":""}

User: "task pending của dự án mobile"
→ {"project_name":"mobile","task_status":"pending","task_assignee":""}

User: "công việc chưa xong của anh Minh ở Website"
→ {"project_name":"Website","task_status":"to do","task_assignee":"Minh"}
"""
